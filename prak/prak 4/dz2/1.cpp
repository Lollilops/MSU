...
// 1 добавления элемента в конец очереди (back),
// 1 чтение первого элемента из очереди без его удаления (front) ,
// 1 удаление первого элемента очереди (pop),
// 1 проверка очереди на пустоту (empty),
// 1 текущее число элементов (size),
// 0 проверка, что очередь целиком заполнена (full).
// 1 вывод информации об элементах очереди без ее изменения (print).
...
class queue: private list_a{
    unsigned int max_size;
public:
    queue(unsigned int i = 10)
    {
        max_size = i;
    };
    ...
    void full(){
        if (max_size == list_a::size())
        {
            cout << "my_queue is full" << endl;
        }
        else
        {
            cout << "my_queue is not full, current number of elements: " << list_a::size() << endl;
        }
    }
};
